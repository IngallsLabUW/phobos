...
CL1.theoretical <- read_csv("example_data/Ingalls_Lab_Standards_MSMS_consensed.csv") %>%
...
all.matches <- experimental.data %>%
  slice(1:20) %>%
  rowwise() %>%
  mutate(CL1.matches = list(ConfLevel1Matches(mz_i = mz, rt_i = rt, col_i = column, z_i = z,
                                          MS2str_i = MS2, ppm_error = 10,
                                          theoretical_db = CL1.theoretical))) %>%
  mutate(Cl2.matches = list(ConfLevel2Matches(mz_i = mz, z_i = z, MS2str_i = MS2,
                                          ppm_error = 10000, theoretical_db = Cl2.matches))) %>%
  ungroup() %>%
  mutate(top_choice_CL1=sapply(CL1.matches, function(cmpd_matches){
    cmpd_matches %>%

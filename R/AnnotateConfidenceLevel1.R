...
AnnotateConfidenceLevel1 <- function(experimental.values, theoretical.values, mz.flexibility, rt.flexibility) {
  ...
  My.Fuzzy.Join <- theoretical.values %>%
    ...
    # Lots of confusion about "MassFeature" vs "compound_experimental" - was broken
    # because compound_experimental wasn't created anywhere, tried changing it to MassFeature with some success
    dplyr::select(MassFeature, compound, mz_experimental, mz_theoretical, rt_experimental, rt_theoretical,
                  column_experimental, column_theoretical, z_experimental, z_theoretical, MS2_experimental, MS2_theoretical)  %>%
    dplyr::arrange(MassFeature)
  ...
  No.Fuzzy.Match <- experimental.values %>%
    ...
    # Anti_join is much clearer here and avoids the 1:nrow(.) problem below
    # anti_join(My.Fuzzy.Join, by="MassFeature") %>%
    dplyr::filter(MassFeature %in% setdiff(1:nrow(.), My.Fuzzy.Join$MassFeature)) %>%
    # Line above does not do what's expected because MassFeature is NOT numeric
    # so checking if it's a number(?) doesn't work
    dplyr::pull()
  ...
  No.CL1.Match <- setdiff(1:nrow(experimental.values),
                          sort(c(unique(Confidence.Level.1$MassFeature),
                                 No.Fuzzy.Match)))

  # Have any compounds been lost? Check for a TRUE output
  all.experimentals <- sort(c(No.Fuzzy.Match, No.CL1.Match, unique(Confidence.Level.1$MassFeature)))
  # Creating an actual "stop" function here because returning a value internally
  # isn't helpful to the actual user
  if(!length(all.experimentals) == length(experimental.values$MassFeature)){
  ...
  No.CL1.Match.df <- experimental.values %>%
    dplyr::filter(MassFeature %in% No.CL1.Match)
  No.Fuzzy.Match.df <- experimental.values %>%
    dplyr::filter(MassFeature %in% No.Fuzzy.Match)
  ...
  Mission.Accomplished <- Confidence.Level.1 %>%
    ...
    dplyr::mutate(mz_experimental = ifelse(is.na(mz_experimental) & !is.na(mz), mz, mz_experimental),
                  rt_experimental = ifelse(is.na(rt_experimental) & !is.na(rt), rt, rt_experimental),
    ...
    # This NA-nuking line of code is a little alarming - usually those NAs
    # are valuable information to carry through the rest of the data. What's

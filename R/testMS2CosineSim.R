...
# uracil.experimental <- c("110.97466, 100; 110.03485, 39.1; 58.02862, 33.5; 68.01299, 26.6; 111.01888, 20.6; 111.05531, 17.5; 68.02917, 14.8; 67.02898, 14.3; 66.00958, 11.7; 79.95618, 11.6; 68.02421, 11.1; 110.04292, 10.8; 110.07133, 8.9; 82.03986, 7; 111.04124, 6; 110.02378, 3.5; 69.04459, 3.2; 83.02393, 3.1; 68.99699, 3; 95.95114, 3; 65.01331, 3; 81.04467, 2.9; 69.01627, 2.8; 83.03205, 2.8; 69.00823, 2.7; 109.94096, 2.6; 82.02856, 2.4; 112.05061, 2.3; 80.96399, 2.3; 65.99735, 2.2; 112.03928, 2; 84.04417, 1.9; 59.03198, 1.2; 66.03379, 1; 96.03178, 1; 70.02859, 1; 51.00574, 0.9; 111.93635, 0.9; 93.04466, 0.8; 111.07485, 0.8; 112.01435, 0.7; 50.0024, 0.7; 112.00274, 0.6; 102.09911, 0.6; 56.62686, 0.6")
# uracil.theoretical <- c("111.01879, 100; 111.04389, 2.4; 111.05523, 1.1; 68.01299, 1")
#
# test.theoretical <- c("118.0854, 7; 118.0864, 100")
# test.experimental <- c("118.0855, 10; 118.0865, 100; 118.0875, 5")
...
###
experimental <- MakeScantable(experimental)

theoretical1 <- MakeScantable(theoretical1)
theoretical2 <- MakeScantable(theoretical2)
theoretical3 <- MakeScantable(theoretical3)
theoretical4 <- MakeScantable(theoretical4)
theoretical5 <- MakeScantable(theoretical5)

complete <- bind_rows(list(a = theoretical1, b=theoretical2, c=theoretical3, d=theoretical4, e=theoretical5), .id = "id")



hc <- complete[,c("mz", "intensity")] %>%
  mutate(intensity = intensity*7.331762e-05) %>% # This value will need to get fiddled with
  dist() %>%
  hclust()
hc %>%
  as.dendrogram() %>%
  plot(ylim=c(0, 0.01))
complete$cluster <- hc %>%
  cutree(h = 0.001) # This value will need to get fiddled with
...
## function that takes in single experimental fragment and assigns to cluster of theoretical data
...
exp_assigned <- experimental %>%
  # slice(1:2) %>%
  rowwise() %>%
  mutate(cluster=AssignCluster(mz, intensity, complete))
complete %>%
  group_by(cluster) %>%
  mutate(mz = mz/7.331762e-05) %>%
  mutate(mean_mz = mean(mz),
